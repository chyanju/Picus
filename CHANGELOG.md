...
## [Unreleased]
- New (advanced API): `picus::solve(&PolyIR, config) -> SolverResult` decides a caller-built constraint system directly (raw `Unsat` / `Sat(model)` / `Unknown`), no R1CS / uniqueness layer. `PolyIR::new` + builder mutators (`push_equality` / `push_disjunction` / `add_disequality` / `add_assignment` / `add_bitsum` / `set_add_field_polys`); `picus` re-exports `PolyIR`, `FfPolyRing`, `PrimeField`, `IrPoly`, `SolverResult`, `UnknownReason` for a `use picus::*` construction path.
...
- Test env var `PICUS_SKIP_PLDI_SMOKE` renamed to `PICUS_TEST_SKIP_SMOKE` (clearer test-only scoping; the `circomlib` smoke test's benchmarks-submodule opt-out). New `docs/testing.md` documents the suite, the smoke test, and the full env-var inventory — Picus reads no runtime env vars (config is TOML/CLI only); the rest are the `cvc5` build-time locators.
