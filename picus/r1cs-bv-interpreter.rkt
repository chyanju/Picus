...
(define (next-symbolic-integer)
    (define-symbolic* r1cs.x (bitvector 254))
    r1cs.x
)
...
(define (interpret-r1cs arg-r1cs arg-xlist)
    ...
    (define sconstraints (for/list ([cnst rconstraints])
        ...
        (define terms-a (cons (bv 0 254) (for/list ([w0 wids-a] [f0 factors-a])
            (bvmul (bv f0 254) (list-ref xlist w0))
        )))
        (define terms-b (cons (bv 0 254) (for/list ([w0 wids-b] [f0 factors-b])
            (bvmul (bv f0 254) (list-ref xlist w0))
        )))
        (define terms-c (cons (bv 0 254) (for/list ([w0 wids-c] [f0 factors-c])
            (bvmul (bv f0 254) (list-ref xlist w0))
        )))
    ...
    (values xlist (cons (bveq (bv 1 254) (list-ref xlist 0)) sconstraints))
)