...
(command-line
    ...
    [("--level") p-level "maximum level of neighboring method: 0 - disable nb | 1 | 2 "
        (begin
            (set! arg-level (string->number p-level))
            (when (! (&& (>= arg-level 0) (<= arg-level 2)))
                (tokamak:exit "level should be 0, 1 or 2")
            )
        )
    ]
