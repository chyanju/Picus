...
fn by_homog_falls_back_to_direct_for_lex() {
    ...
    // Direct for other orders, and the label records the route actually
    // executed (each impl records inside `compute`, so a fallback is
    // visible in telemetry rather than hidden behind the chosen name).
    assert_eq!(
        last_dispatched_algorithm(),
        Some("buchberger-direct"),
        "Lex under ByHomog executes (and records) the Direct route"
    );
}
